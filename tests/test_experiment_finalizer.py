...
def test_finalizer_may_bypass_only_unchanged_legacy_publish_gate_findings() -> None:
    ...
    assert "UG_PUBLISH_GATE_BASELINE=<pre-edit-storage-file>" in FINALIZATION_PROMPT
    assert "Never use\n   `UG_PUBLISH_GATE_DISABLED`" in FINALIZATION_PROMPT
    assert "never permits a defect in this run's block" in FINALIZATION_PROMPT
...
def test_prompt_has_selection_pipeline_and_production_routing() -> None:
    ...
    assert "AUTOMATED ATLASSIAN IDENTITY" in FINALIZATION_PROMPT
    assert "pass `--as-bot` on every command" in FINALIZATION_PROMPT
    assert "Never fall back to personal" in FINALIZATION_PROMPT
    assert "AUTOMATED RESPONSE STYLE" in FINALIZATION_PROMPT
